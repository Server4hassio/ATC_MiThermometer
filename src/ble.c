...
void set_adv_data(void) {
	...
	// Delta-on-change filter: keep previous adv payload if reading hasn't moved enough.
	// Forces a refresh every ADV_DELTA_FORCE_REFRESH calls so battery/counter still update.
	#define ADV_DELTA_TEMP_X100      50    // 0.5 C  (units of 0.01 C)
	#define ADV_DELTA_HUMI_X100      500   // 5.0 %  (units of 0.01 %)
	#define ADV_DELTA_FORCE_REFRESH  60    // force payload refresh every N calls
	static s16 last_pub_temp;
	static s16 last_pub_humi;
	static u8  delta_skip_cnt;
	static u8  delta_inited;
	if (delta_inited && adv_buf.data_size && delta_skip_cnt < ADV_DELTA_FORCE_REFRESH) {
		s16 dt = measured_data.temp - last_pub_temp;
		s16 dh = measured_data.humi - last_pub_humi;
		if (dt < 0) dt = -dt;
		if (dh < 0) dh = -dh;
		if (dt < ADV_DELTA_TEMP_X100 && dh < ADV_DELTA_HUMI_X100) {
			delta_skip_cnt++;
			return; // keep prior adv_buf.data, don't regenerate payload
		}
	}
	last_pub_temp = measured_data.temp;
	last_pub_humi = measured_data.humi;
	delta_skip_cnt = 0;
	delta_inited = 1;
#endif
